<template>
  <v-app>
    <v-container>
      <v-select :items="protestDatasets.map((data) => data.name)" label="Location" v-model="name"></v-select>

      <v-layout justify-space-around class="mt-4 mb-10">
        <v-card class="rounded-lg" style="border-bottom: 5px solid #fbdf81">
          <v-card-title>
            <v-icon class="mr-2">{{ mdiCalendarToday }}</v-icon>
            Today Casualties
          </v-card-title>
          <v-card-text class="text-h5">{{ protestDatasets[getDataIndex].totalCasualties }} </v-card-text>
        </v-card>

        <v-card class="rounded-lg" style="border-bottom: 5px solid #f4772f">
          <v-card-title>
            <v-icon class="mr-2">{{ mdiCalendarWeek }}</v-icon>
            Weekly Casualties
          </v-card-title>
          <v-card-text class="text-h5">{{ protestDatasets[getDataIndex].weeklyCasualties }}</v-card-text>
        </v-card>

        <v-card class="rounded-lg" style="border-bottom: 5px solid #f83145">
          <v-card-title class="mr-2">
            <v-icon>{{ mdiCalendarMonth }}</v-icon>
            Total Casualties
          </v-card-title>
          <v-card-text class="text-h5">{{ protestDatasets[getDataIndex].totalCasualties }} </v-card-text>
        </v-card>
      </v-layout>

      <BarChart :chartData="protestDatasets[getDataIndex]" :height="200" />
    </v-container>
  </v-app>
</template>

<script lang="ts">
import { Vue, Component } from 'vue-property-decorator';
import BarChart from '@/components/charts/BarChart';
import { protestDatasets, ProtestDataset } from '@/store/dummyData';
import { mdiCalendarToday, mdiCalendarWeek, mdiCalendarMonth } from '@mdi/js';

@Component({ components: { BarChart } })
export default class Protest extends Vue {
  protestDatasets: ProtestDataset[] = protestDatasets;
  mdiCalendarToday: string = mdiCalendarToday;
  mdiCalendarWeek: string = mdiCalendarWeek;
  mdiCalendarMonth: string = mdiCalendarMonth;

  name: string = protestDatasets.map((data) => data.name)[0];

  get getDataIndex(): number {
    return protestDatasets.findIndex((data) => data.name === this.name);
  }
}
</script>
